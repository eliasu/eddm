...
/***/ "./resources/js/LightBoxWithSwiper.js":
/*!********************************************!*\
  !*** ./resources/js/LightBoxWithSwiper.js ***!
  \********************************************/
...
exports.LightBoxWithSwiper = void 0;
...
var LightBoxWithSwiper =
/** @class */
function () {
  function LightBoxWithSwiper(imgSelector, additionalConfig) {
    var _this = this;
    ...
    this.lightbox = this.createClassedFilledElement("lightbox-container", this.lightboxAndswiperMarkup);
    this.swiperWrapper = this.lightbox.querySelector(".swiper-wrapper");
    this.close = this.lightbox.querySelector(".close");
    this.lightboxImages = document.querySelectorAll(imgSelector);
    this.close.addEventListener("click", function (_a) {
      var target = _a.target;

      _this.lightbox.classList.remove("open");

      _this.removeLightbox(1250);

      document.body.classList.remove("overflowHidden");
    });
    this.lightboxImages.forEach(function (element, index) {
      // element.addEventListener("click", this.handleClick.bind(this), false);
      element.addEventListener("click", function (evt) {
        document.body.append(_this.lightbox); // console.log(evt);

        _this.initSwiper(index);

        _this.lightbox.classList.add("open");
      });
      var imgClone = element.cloneNode();

      var slide = _this.createClassedFilledElement("swiper-slide");

      slide.appendChild(imgClone);

      _this.swiperWrapper.appendChild(slide);
    });
  }

  LightBoxWithSwiper.prototype.createClassedFilledElement = function (className, htmlContent, tag) {
    if (className === void 0) {
      className = undefined;
    }
    ...
    if (htmlContent) el.insertAdjacentHTML("afterbegin", htmlContent);
    return el;
  };

  LightBoxWithSwiper.prototype.initSwiper = function (index) {
    var config = __assign(__assign({}, this.additionalConfig), {
      modules: [swiper_1.Navigation],
      initialSlide: index,
      // If we need pagination
      pagination: {
        el: ".lightbox-container > .lightbox-swiper > .swiper-pagination"
      },
      // Navigation arrows
      navigation: {
        nextEl: ".lightbox-container > .lightbox-swiper > .swiper-button-next",
        prevEl: ".lightbox-container > .lightbox-swiper > .swiper-button-prev"
      },
      // And if we need scrollbar
      scrollbar: {
        el: ".lightbox-container > .lightbox-swiper > .swiper-scrollbar"
      }
    });

    document.body.classList.add("overflowHidden"); // init Swiper:

    this.swiper = new swiper_1["default"](".lightbox-container > .lightbox-swiper", config);
  };

  return LightBoxWithSwiper;
}();

exports.LightBoxWithSwiper = LightBoxWithSwiper;
...
/******/ 	var __webpack_exports__ = __webpack_require__("./resources/js/LightBoxWithSwiper.js");
/******/ 	
/******/ })()
;
//# sourceMappingURL=LightBoxWithSwiper.js.map